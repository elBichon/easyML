...
def read_dataset(name,separator):
    ''' This function enables to read a given csv dataset
        any kind of separator can be specified
    '''
    data = pd.read_csv(name, sep=separator)
    return data

def convert_to_log(data, min_column):
    data = math.log(data+min_column+1)
    return data
